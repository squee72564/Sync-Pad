...
The Permify bootstrap prints a `PERMIFY_SCHEMA_VERSION=...` value. Add that value to `api/.env` and `websocket/.env`, then restart `pnpm dev` so the services use the schema version for authorization writes and checks.
...
For a full deployment outline, see [Production deployment](./docs/production-deployment.md).
